...
!------------------------------------------------------------------------------
  ...
  WRITE(logUnit,'(/A)' ) " ------------------------------------------------------------------------------"
  WRITE(logUnit,'(A)')   "                           SETUP KEDF"
  WRITE(logUnit,*) "kinetic energy density functional number is ", kinetic
  WRITE(logUnit,*) "( 1=TF, 2=VW, 3=TF+VW, 4=WT,  5=WGC,  "
  WRITE(logUnit,*) "  7=LQ, 8=HQ, 10=CAT,  11=HC, 12=WGCD,"
  WRITE(logUnit,*) "  15=GGA, 16=WGCD+GGA, 17=EVT, 18=EVC )"
  ...
  WRITE(logUnit,'(A,3I5)') " qTable dimension: ", k1G, k2G, k3G
...
!------------------------------------------------------------------------------
  CASE(1) ! TF
    ...
    IF(rankGlobal==0) & !VVK ADDED, 06 SEP 2015
    WRITE(logUnit,'(A,F10.4)') " Coefficient for TF KEDF is ", lambda
!------------------------------------------------------------------------------
  CASE(2) ! VW
    ...
    IF(rankGlobal==0) & !VVK ADDED, 06 SEP 2015
    WRITE(logUnit,'(A,F10.4)') " Coefficient for VW KEDF is ", mu
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
  CASE(11) ! Huang-Carter KEDF
    ...
    WRITE(logUnit,'(A,F12.6)') " ALPHA  in HC10 is set to ", alpha
    WRITE(logUnit,'(A,F12.6)') " BETA   in HC10 is set to ", beta
    WRITE(logUnit,'(A,F12.6)') " Sum of alpha and beta is ", alpha+beta
    ENDIF !VVK ADDED, 06 SEP 2015
