...
!------------------------------------------------------------------------------
  ...
  logFile = TRIM(systemName)    // defaultLog !VVK
...
!--> VVK
!
! 09 JULY 2015: The block below is commented and moved below!!!
!  IF(TRIM(systemName).eq.'ofdft') THEN !do nothing for PR@QE, logUnit=6
!    logUnit = 6
!  ELSE !open logUnit=61 file for PR
!    !flush(6)
!    logUnit = 61 !logUnit ! we need to open logUnit file to avoid multiple copies
!                          ! of output for parallel version
!  ! Open the Log file before CALL ReadOptions
!    IF (rankGlobal==0) THEN
!      OPEN(UNIT=logUnit, ACCESS="sequential", ACTION="write", BLANK="null", &
!           DELIM="none", FILE=TRIM(logFile), FORM="formatted",& 
!           IOSTAT=fileStatus, PAD="no", POSITION="rewind", STATUS="replace")
!
!      IF (fileStatus/=0) THEN
!        WRITE(*,*)'The file ',TRIM(logFile),' had a problem opening.'
!        WRITE(*,*)'And I cannot even begin to imagine what it might be.'
!        STOP
!      END IF
!    ENDIF
!  ENDIF
!! VVK: do not forget comment/uncomment CLOSE(logUnit) statement in Report.f90
!!
!! VVK: from profess2.0 InitializeInputs.f90
!  ! First read all the options.  This also might overwrite the error and 
!  ! output file names.
!!#ifdef __USE_PARALLEL
...
!<-- VVK
  ...
  IF (output_all_files==1 .OR. & 
    ...
    IF(output_all_files==1) THEN
      ...
      WRITE(logFile, '(a,a)') TRIM(logFile), TRIM(ADJUSTL(tempName)) !VVK
      outputFile=TRIM(ADJUSTL(outputFile))
      logFile=TRIM(ADJUSTL(logFile)) !VVK
...
!--> VVK
!
! 09 JULY 2015: The block from above is moved here:
  !IF(TRIM(systemName).eq.'ofdft') THEN !do nothing for PR@QE, logUnit=6
  IF(PRatQE) THEN !do nothing for PR@QE, logUnit=6
    !logUnit = 6
  ELSE !open logUnit=61 file for PR
    !flush(6)
!    logUnit = 61 !logUnit ! we need to open logUnit file to avoid multiple copies
...
!#endif
! 
! 09 JULY 2015: open scratch file to redirect some output from RhoOptN.f90 for
! PR@QE from unit 6 to scratch due to it is very long:
!
  IF(PRatQE) THEN ! Open scratch file
     open(scratchUnit, status='SCRATCH') 
  ENDIF
!
!<-- VVK
...
! VVK-->
  !write(logUnit,*) 'VVK logUnit test write, rankGlobal=',rankGlobal

  !WRITE(outputUnit,*) "******************************************************************************" 
  !WRITE(outputUnit,*) "                            WELCOME TO PROFESS                   "
  !WRITE(outputUnit,*) "           (PRinceton Orbital-Free Electronic Structure Software) "
  !WRITE(outputUnit,*) "******************************************************************************" 
  !WRITE(outputUnit,11) REPEAT("*", 78)
  !WRITE(outputUnit,12) "                            WELCOME TO PROFESS                   "
  !WRITE(outputUnit,12) "           (PRinceton Orbital-Free Electronic Structure Software) "
  !WRITE(outputUnit,12) "     PROFESS/Profess@Quantum-Espresso v3.0@5.1.2 APRIL 2015 (QTP/UF)"
  !WRITE(outputUnit,11) REPEAT("*", 78)
  11 FORMAT(A)
  12 FORMAT("*", A75, " *")
!VVK <--
