...
!------------------------------------------------------------------------------
  ...
  COMPLEX(KIND=DP), DIMENSION(k1G, k2G, k3G) :: rhoRecip,trafo1,trafo2 !VVK: trafo1 and trafo2 are used for FFT_2 as well 
...
#ifdef __FFTW2 !--> VVK
    gradtot(:,:,:,1) = FFT_2(imag*qVectors(:,:,:,1)*rhoRecip)
    gradtot(:,:,:,2) = FFT_2(imag*qVectors(:,:,:,2)*rhoRecip)
    gradtot(:,:,:,3) = FFT_2(imag*qVectors(:,:,:,3)*rhoRecip)
#else
  trafo1 = imag*qVectors(:,:,:,1)*rhoRecip
  ...
  Else
    ...
    ! old FFT:
    rhoRecip = FFT_2(up)
    gradup(:,:,:,1) = FFT_2(imag*qVectors(:,:,:,1)*rhoRecip)
    gradup(:,:,:,2) = FFT_2(imag*qVectors(:,:,:,2)*rhoRecip)
    gradup(:,:,:,3) = FFT_2(imag*qVectors(:,:,:,3)*rhoRecip)
#else
    ...
    ! old FFT:
    rhoRecip = FFT_2(up)
    graddn(:,:,:,1) = FFT_2(imag*qVectors(:,:,:,1)*rhoRecip)
    graddn(:,:,:,2) = FFT_2(imag*qVectors(:,:,:,2)*rhoRecip)
    graddn(:,:,:,3) = FFT_2(imag*qVectors(:,:,:,3)*rhoRecip)
#else
...
#ifdef __FFTW2 !--> VVK
      trafo1 = FFT_2(rhonow(:,:,:,ispin,2))
      trafo2 = FFT_2(rhonow(:,:,:,ispin,3))
#else
...
!  WRITE(*,'(3F12.6)') PBEStress(3,:)*auToGPa
  ...
  CALL StopClock("PBEStress#endif         !<-- VVK")
