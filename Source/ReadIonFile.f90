...
!------------------------------------------------------------------------------
  ...
  USE MPI_Functions, ONLY: rankGlobal !VVK
...
! VVK ADDED
  REAL(kind=DP) :: &
    cellScale                  ! to scale cell dimensions
! VVK END
...
! VVK ADDED
          READ(inputUnit,*,iostat=fileStatus) cellScale

          IF (fileStatus/=0) THEN
           BACKSPACE(inputUnit,iostat=fileStatus)
            cellScale = 1._DP
          END IF
          IF(cellScale.ne.1._DP) then
            WRITE(message,*) 'VVK: cellScale set to cellScale =',cellScale
            CALL WrtOut(6,message)
            WRITE(message,*) 'VVK: cell%cellReal scaled'
            CALL WrtOut(6,message)
            cell%cellReal = cell%cellReal * cellScale
          END IF
! VVK END
          IF (fileStatus/=0) THEN
...
! VVK ADDED
! FEB 2015, scale lattice if needed
          IF(cellScale.ne.1._DP) then
            WRITE(message,*) 'VVK: cellScale set to cellScale =',cellScale
            CALL WrtOut(6,message)
            WRITE(message,*) 'VVK: cell%cellReal scaled'
...
! --> VVK ADDED 
      IF(cellScale.ne.1._DP) THEN
        cell%ionTable(i) % coord = cell%ionTable(i) % coord * cellScale !added VVK - scale ion Cartesian coordinates
      END IF
! <-- VVK END
...
! --> VVK ADDED
    IF(cellScale.ne.1._DP) THEN
      write(message,*) 'VVK: Ion Cart. coord. scaled by cellScale=',cellScale !<-- ADDED VVK 
      CALL WrtOut(6,message)
    END IF
! <-- VVK END
  END IF
...
! --> VVK ADDED, MAR 2015
      IF(cellScale.ne.1._DP) THEN
        cell%ionTable(i) % coord = cell%ionTable(i) % coord * cellScale !added VVK - scale ion "in bohr" coordinates
      END IF
! <-- VVK END
      ! This is technically a projection along the cell axes. Cute, eh?
...
! --> VVK ADDED
    IF(cellScale.ne.1._DP) THEN
      write(message,*) 'VVK: Ion "in bohr" coord. scaled by cellScale=',cellScale !<-- ADDED VVK 
      CALL WrtOut(6,message)
    END IF
! <-- VVK END
  END IF
...
!------------------------------------------------------------------------------
  ...
  IF(rankGlobal==0) THEN !VVK ADDED, 06 SEP 2015
  WRITE(logUnit,'(/A)' ) " ------------------------------------------------------------------------------"
  WRITE(logUnit,'(A)')   "                           READ PSEUDOPOTENTIAL"
  ENDIF !VVK ADDED, 06 SEP 2015
    ...
    ! so we can read to the end.)
      ...
      IF(psp%type == 1) THEN
        ...
        ionCharge = NINT( &
          ...
          (4.0_DP * pi))
! --> VVK ADDED
 IF(.false.) then !VVK-fix this print for parallel version
 write(message,1010) ' VVK: psp%potValues(2),psp%potValues(1)=',psp%potValues(2),psp%potValues(1)
 CALL WrtOut(6,message) 
 write(message,1010) ' VVK: psp%maxG,REAL(psp%numPoints-1,DP)=',psp%maxG,REAL(psp%numPoints-1,DP)
 CALL WrtOut(6,message)
 write(message,1010) ' VVK: (-pot(2)-pot(1))*maxG/(psp%numPoints-1)^2/4/Pi=', & 
   - (psp%potValues(2) - psp%potValues(1)) * (psp%maxG / REAL(psp%numPoints-1,DP))**2 / (4.0_DP * pi)
 CALL WrtOut(6,message)
 write(message,*) 'VVK: ionCharge=',ionCharge
 CALL WrtOut(6,message)
 ENDIF
1010 format(a,2d15.7)
!       ionCharge = 3.
! <-- VVK END
...
!                            i, ',  of ionCharge=', ionCharge
  ...
  WRITE(logUnit,*) "Total electron number in ReadPseudo is ", cell%numEle
...
!------------------------------------------------------------------------------
  ...
  IF (fileStatus/=0) THEN
    ...
    IF(rankGlobal==0) & !VVK ADDED, 06 SEP 2015
    WRITE(logUnit,*)'Please make sure this density file does exist.'
    STOP
  ELSE IF(fileStatus==0) THEN 
    IF(rankGlobal==0) & !VVK ADDED, 06 SEP 2015
    WRITE(logUnit,'(A,A)') " Read in the denisty file from ",densityFile
  END IF
