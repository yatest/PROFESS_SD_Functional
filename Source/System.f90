...
! space have are sliced in the same way.
  ...
  REAL(KIND=DP), DIMENSION(21) :: energy = 0.0_DP ! VVK: changed: 9 --> 21       
  ...
  ! VVK:
  ! 7) Thomas-Fermi --> Fs
  ! ETable(7)=FreeE, i.e. non-interacting free-energy Fs
  ! ETable(10)=mcGGA or GGA KE (vW+Pauli) <-- obsolete, not used
  ! ETable(11)=vW (if pauli=/0)           <-- obsolete, not used
  ! ETable(11)=vW (if pauli=/0)           <-- obsolete, not used
  ! ETable(12)= Temper * Entropy (T*Ss) (electronic temperature x noninteracting entropy)
  ! ETable(13)= second term in non-interacting free-energy functional T*S0
  ! ETable(14)= negative part of (T*Ss), if any
  ! ETable(15)= X energy for the case of X+C functionals
  ! ETable(16)= C energy for the case of X+C functionals
  ! ETable(17)= XC internal energy (implemented for KSDT functional only)

  REAL(DP) rhomax,rhomin,s2max,s2min,p1max,p1min,potmax,potmin !VVK added to be able to use it in Output
  LOGICAL :: PRatQE ! VVK: variable, it is true for Profess@QE interface
...
!------------------------------------------------------------------------------
  ...
  WRITE(logUnit,*) '(System) Machine Precision              : ', machprec
  ...
  WRITE(logUnit, *) "Dimension of rho for this processor : "
  WRITE(logUnit, *) "xmin, xmax : ",xmin, xmax
  WRITE(logUnit, *) "ymin, ymax : ",ymin, ymax
  WRITE(logUnit, *) "zmin, zmax : ",zmin, zmax
  ...
  IF( cell%vol .EQ. 0.0_DP ) THEN
    WRITE(logUnit,*) "The cell volume is 0, check the lattice vectors."
    STOP
  ENDIF
  ...
  IF(rankGlobal==0) THEN !VVK ADDED, 06 SEP 2015
  WRITE(logUnit,*) "Total electron number = ",SUM(cell%elementTable%chargeTot)
  WRITE(logUnit,*) "Uniform rhoR = ", rhoR(xmin,ymin,zmin,1)
  ENDIF !VVK ADDED, 06 SEP 2015
  ...
  IF(numspin == 2) Then
     DO isp = 1,2
        ...
        IF(rankGlobal==0) & !VVK ADDED, 06 SEP 2015
        WRITE(logUnit,*) "Electron number for spin ", isp, " is ", sumrho
     ENDDO
  ENDIF
...
!------------------------------------------------------------------------------
  ...
  IF(status/=0) THEN
    WRITE(logUnit,*) "Problem deallocating grids in CleanSystem"
    STOP
  ENDIF
