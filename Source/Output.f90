...
!------------------------------------------------------------------------------
  ...
  12 FORMAT(A1, A76, A1)
! 121 FORMAT(A1, A77, A1) !VVK

                       !>> FUNCTION BODY <<!

  ! Output the lattice vectors, convert to A.
!--> VVK: replace as it was before in profess 2.0 
!  WRITE(outputUnit, *) " "
!  WRITE(outputUnit, *) "------------------------------------------------------------------------------"
...
!  END DO
  ...
  DO i = 1, 3
    ...
    IF(i == 2) comp = "LA 2"
    IF(i == 3) comp = "LA 3"
    WRITE(outputUnit, '(A1, 1X, A4, 1X, 3(ES18.8, 1X), A14)')& 
...
!    WRITE(outputUnit, '(1X, A8, 1X, 3(ES18.8, 1X))') comp(1:8), & !23 JUN 2015: F15.6 --> ES18.8
!       stress(i,:) * auToGPa 
!  END DO
  WRITE(outputUnit, 12) border, REPEAT(",", 76), border
...
!  !WRITE(outputUnit, *) "--------------------------------------------------"
!  WRITE(outputUnit, *) "------------------------------------------------------------------------------" !VVK
!  WRITE(outputUnit, *) "                               STRESS (a.u.)" !VVK
!  WRITE(outputUnit, *) "------------------------------------------------------------------------------" !VVK
!  DO i = 1, 3
!    IF(i == 1) comp = "ST_AU  1"
!    IF(i == 2) comp = "ST_AU  2"
!    IF(i == 3) comp = "ST_AU  3"
!    WRITE(outputUnit, '(1X, A8, 1X, 3(ES18.8, 1X))') comp(1:8), stress(i,:) !23 JUN 2015: ES18.10 --> ES18.8
!  END DO
  WRITE(outputUnit, 12) border, REPEAT(",", 76), border
  WRITE(outputUnit, 12) border, "STRESS (a.u.)" // REPEAT(" ", 31), border
  WRITE(outputUnit, 12) border, REPEAT(",", 76), border
  WRITE(outputUnit, 12) border, &
                      REPEAT(" ", 6) // "x" // REPEAT(" ", 18) // "y" // &
                      REPEAT(" ", 18)// "z" // REPEAT(" ", 25), &
...
! VVK ADEDD: PrintPressure
  WRITE(outputUnit, 12) border, REPEAT(",", 76), border
!    WRITE(outputUnit, '(A1,1X,A15,1X,f14.8,45x,A1)') &
!    ';','Pressure(GPa) =',-(stress(1,1)+stress(1,1)+stress(1,1)) * auToGPa/3._DP,';'
    WRITE(outputUnit, '(A1,1X,A15,1X,ES18.8,A6,35x,A1)') & !23 JUN 2015: f15.4 --> ES18.8
    ';','     Pressure =',-(stress(1,1)+stress(2,2)+stress(3,3)) * auToGPa/3._DP,' GPa  ',';' !10 OCT 2018: DMR bugfix
    WRITE(outputUnit, '(A1,1X,A15,1X,f18.8,A6,35x,A1)') & !23 JUN 2015: f15.8 --> f18.8
    ';','   CellVolume =',Volume(cell%cellReal)*bohr**3,' Ang^3',';'
  WRITE(outputUnit, 12) border, REPEAT(",", 76), border
  WRITE(outputUnit, *) ""
! VVK END
...
!------------------------------------------------------------------------------
 ...
 IMPLICIT NONE
   ...
   INTEGER :: unitOuttmp !VVK

                         !>> FUNCTION BODY <<!
!--> VVK 
! all output to the standard unit is redirected to the logUnit
! if we really need some output to unit 6 (as in case of PR@QE interface)
! we use unit 60 and here it goes to 6
!
! still not finished... for PR the output goes to the standard unit 6 and to the
! logUnit=61
!
   IF (outputRank==0) THEN
     unitOuttmp = unitOut
!     if((unitOut.eq.6).and.(logUnit.eq.61)) then !for PR redirect output from 6
!                                                !to logUnit=61
!       unitOuttmp = logUnit
!     endif
   ENDIF
...
!
!   if(unitOut.eq.6) then 
!     unitOuttmp = logUnit
!   elseif(unitOut.eq.60) then
!     unitOuttmp = 6
!   else
!     unitOuttmp = unitOut
!   endif
!<-- VVK
! VVK: below unitOut --> unitOuttmp

   IF (outputRank==0) THEN
     IF (PRESENT(nextLine)) THEN
       IF (nextLine) THEN
         WRITE(unitOuttmp,'(A)') TRIM(msg)
       ELSE
         WRITE(unitOuttmp,'(A)',ADVANCE="NO") TRIM(msg)
       ENDIF
     ELSE
       WRITE(unitOuttmp,'(A)') TRIM(msg)
     ENDIF
...
#endif
  !
  !VVK: below "WrtOut(6," is replaced with "WrtOut(logUnit,"
  CALL WrtOut(logUnit," ")
  CALL WrtOut(logUnit," -------------------------------- WARNING ------------------------------------")
  CALL WrtOut(logUnit," PROFESS Stop for the following reason:")
  IF(PRESENT(message)) THEN
    CALL WrtOut(logUnit, message)
  ENDIF
  CALL WrtOut(logUnit," Find out the reason and restart calculation again.")
  CALL WrtOut(logUnit," -----------------------------------------------------------------------------")
