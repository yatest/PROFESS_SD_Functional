...
!------------------------------------------------------------------------------
  ...
  IF((m1G .LE. 0) .OR. (m2G .LE. 0) .OR. (m3G .LE. 0)) THEN
    WRITE(logUnit,*) " check the value of m1G, m2G, m3G."
    STOP
  ENDIF
  ...
  WRITE(logUnit,'(A,3I8)') " (GB INFO) FFT size      : ", n1G, n2G, n3G 
  WRITE(logUnit,'(A,I8)')  " (GB INFO) FFT offset    : ", n3Goff
  WRITE(logUnit,'(A,3I8)') " (GB INFO) FFT G size    : ", k1G, k2G, k3G 
  WRITE(logUnit,'(A,I8)')  " (GB INFO) FFT G offset  : ", k3Goff
...
!------------------------------------------------------------------------------
  ...
  IF(m123G > 0) THEN
    ...
    WRITE(logUnit,*) " dV of each real space grid is now ", cell%dV
  ENDIF
  ...
  ! print out the cell
  IF(rankGlobal==0) THEN !VVK ADDED, 06 SEP 2015
  WRITE(logUnit,'(A,3(ES12.3))') " Cell length (Bohr) ", cell%lengthA, cell%lengthB, cell%lengthC
  WRITE(logUnit,'(A,3(ES12.3))') " Angle     (Degree) ", cell%angleAlpha, cell%angleBeta, cell%angleGamma
  WRITE(logUnit,'(A,ES12.3)')    " Volume    (Bohr^3) ", cell%vol 
  ENDIF !VVK ADDED, 06 SEP 2015
