...
SUBROUTINE vWGTF(rho, potential, calcEnergy, energy, model,TVW,TTF) 
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
  ...
  REAL(kind=DP), Dimension(:,:,:), INTENT(out) :: &
    potential           ! dE/drho, i.e., kinetic energy potential
  REAL(kind=DP), INTENT(out) :: TVW,TTF !VW,TF KE, VVK added
...
!>>>>>>>>>>>>>>>>>>  functional body <<<<<<<<<<<<<<<<<<<<<<
    ...
    IF (calcEnergy) TVW = energy           !VVK: T_VW for test
    IF (calcEnergy) TTF = CTF * SUM(rho53) !VVK: T_TF for test
