...
!------------------------------------------------------------------------------
  ...
  WRITE(logUnit, '(A,3I5)') " Global FFT Dimension : ", totX, totY, totZ
  IF(rankGlobal==0) THEN
    WRITE(logUnit, '(A,3I5)') " (Setup System) Global FFT Dims.         : ", totX, totY, totZ !VVK 6 --> logUnit
  ENDIF
...
#ifdef __FFTW2 !--> VVK
    CALL PlanFFT_2(totX, totY, totZ) !VVK
#endif         !<-- VVK
#else
    ! MEASURE WILL INTRODUCE RANDOM SMALL ERRORS AT FIRST. (mohan note)
  ...
  IF(rankGlobal==0) THEN !VVK ADDED, 06 SEP 2015
  WRITE(logUnit,*) "Rank/Size in Global Communicator     : ", rankGlobal, "/", sizeGlobal
  WRITE(logUnit,*) "Rank/Size in Global FFT Communicator : ", rankGFFT, "/", sizeGFFT
  ENDIF !VVK ADDED, 06 SEP 2015
!  WRITE(logUnit,*) "Processor number for Global FFT  : ", numProcGFFT
!  WRITE(logUnit,*) "Index in Global FFT Communicator : ", indGFFT
  ...
  IF(rankGlobal==0) THEN !VVK ADDED, 06 SEP 2015
  WRITE(logUnit,"(A,I5)") " Parallel FFT, totX    = ",totX
  WRITE(logUnit,"(A,I5)") " Parallel FFT, totY    = ",totY
  WRITE(logUnit,"(A,I5)") " Parallel FFT, locZ    = ",locZ
  WRITE(logUnit,"(A,I5)") " Parallel FFT, locZoff = ",locZoff
  ENDIF !VVK ADDED, 06 SEP 2015
