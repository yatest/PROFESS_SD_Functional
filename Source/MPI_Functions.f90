...
!------------------------------------------------------------------------------
  ...
  !USE,INTRINSIC::iso_c_binding !VVK
  USE OutputFiles
...
!--> VVK: commented and replaced:
  ...
  !INTEGER(C_INT32_T) :: MPI_GFFT_WORLD ! Communicator name for global FFT. !VVK
!<-- VVK
...
SUBROUTINE InitializeMPI(callInit) !VVK: () --> (callInit)
!------------------------------------------------------------------------------
! DESCRIPTION:
...
!------------------------------------------------------------------------------
  ...
  IMPLICIT NONE
                      ...
                      !>> EXTERNAL VARIABLES <<! !VVK
  LOGICAL, INTENT(IN) :: callInit !VVK
  ! call MPI_INIT or not !VVK
...
!<-- VVK
  ...
  IF (rankGlobal==0) WRITE(*,*) "Parallel version of QTP-PROFESS, total Processor Number is ", sizeGlobal !VVK
  IF (mpiErr/=MPI_SUCCESS) STOP "**PROBLEMS FINDING RANK OF QTP-PROCESS**" !VVK
  ...
  IF( rankGlobal == 0 ) THEN
    !WRITE(*,*) "Parallel version of QTP-PROFESS" !VVK
  ENDIF
#else
  WRITE(*,*) "Serial version of QTP-PROFESS" !VVK
#endif
