...
!------------------------------------------------------------------------------
  ...
  USE FXC_KSDT, ONLY: KSDTXCStress ! KSDT XC stress; VVK
  USE FXC_PDW00, ONLY: PD00XCStress ! PDW00 XC stress; VVK
  USE FS_TF, ONLY: TTF1Stress !finite-T TF stress; VVK
  USE FS_GGA, ONLY: TGGAStress !finite-T TF stress; VVK
...
#ifdef __FFTW2 !--> VVK
    rhoRecip_SI(:,:,:,isp) = FFT_2(rhoR(:,:,:,isp))
#else
  ...
  SELECT CASE(kinetic)
    ...
    CASE(2) 
      ! Von Weiszacker (vW)
       DO isp = 1, numSpin
#ifdef __FFTW2 !--> VVK
          stress = stress + tmp2*VWStress(FFT_2(SQRT(tmp1*rhoR(:,:,:,isp))), m123G)
#else
    ...
    CASE(3) 
      ...
      stress = stress + TFStress(cell%vol, energy(7))
       DO isp = 1, numSpin
          stress = stress &
#ifdef __FFTW2 !--> VVK
               + tmp2*VWStress(FFT_2(SQRT(tmp1*rhoR(:,:,:,isp))), m123G)
#else
...
! --> VVK ADDED
    CASE(1010) ! TTF1
          stress = stress + TTF1Stress(cell%vol, energy(7), rhoR(:,:,:,1))
    CASE(1013:1125) ! VT84FN, etc.. new implementation of the GGA functionals
          stress = stress + TGGAStress(cell%vol, rhoR, rhoR(:,:,:,1), rhoRecip_SI(:,:,:,1),kinetic)
! <-- VVK END
...
! --> VVK ADDED: PDW2000 XC
    CASE(3) ! T-dep. Correlation
      stress = stress + PD00XCStress(cell%vol, rhoR, rhoR(:,:,:,1))

    CASE(4) ! T-dep. Correlation
      stress = stress + KSDTXCStress(cell%vol, rhoR, rhoR(:,:,:,1))
! <-- VVK END
    CASE DEFAULT ! This case should never occur.
