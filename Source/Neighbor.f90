...
!------------------------------------------------------------------------------
  ...
  !VVK: Output below: CALL WrtOut (6,msg) --> CALL WrtOut (logUnit,msg) !VVK: "(6,msg)" --> "(logUnit,msg)"
...
!   CALL WrtOut(6," nnDist < 0, then Skipped NNDistCheck (bin).")
  ...
  IF (SIZE(cell%ionTable,1) == 1) then
    ...
    !CALL WrtOut (6,msg)
    CALL WrtOut (logUnit,msg) !VVK: "(6,msg)" --> "(logUnit,msg)"
    NNDist_bin = 1
    RETURN
  ENDIF
  ...
  CALL WrtOut (logUnit,message) !VVK: "(6,msg)" --> "(logUnit,msg)"

! VVK: commented line below
  !test = .TRUE. 
  test = .FALSE. !VVK Added
  IF(test) THEN
...
!------------------------------------------------------------------------------
  ...
  IF (nnDist <= 0 ) THEN 
    ...
    CALL WrtOut(logUnit,"nnDist < 0, then Skipped NNDistCheck.") !VVK
    minDistance = 1e6
    NNDist_noBin = 1
    RETURN
  ...
  ELSE
    ...
    CALL WrtOut(6, "Check NNdist...") !VVK
    ...
    !CALL WrtOut(6,message)
    CALL WrtOut(logUnit,message) !VVK
