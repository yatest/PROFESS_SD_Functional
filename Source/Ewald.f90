...
!------------------------------------------------------------------------------
  ...
  !USE FOURIER, ONLY : FFT ! The Fast Fourier Transform routine. !VVK
#endif         !<-- VVK
...
!------------------------------------------------------------------------------
  ...
  USE CBSpline, ONLY : BSplineProduct_old !VVK
...
#ifdef __FFTW2 !--> VVK
  !a)using BSplineProduct_old
  !CCStructure = BSplineProduct_old(FFT(qIonTable))
  ! or
  !b) using BSplineProduct
  tmpRecip = FFT_2(qIonTable)
  !CCStructure = BSplineProduct_old(tmpRecip)
