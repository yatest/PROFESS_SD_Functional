...
!------------------------------------------------------------------------------
  ...
  ! VVK output below outputUnit--> logUnit
  ! VVK: for PR@QE logUnit is changed to scratchUnit (in profess_sub.f90) after first MD step 
  USE OutputFiles, ONLY: scratchUnit !VVK
  USE SYS, ONLY : PRatQE ! VVK: variable, it is true for Profess@QE interface
  USE MPI_Functions, ONLY: rankGlobal !VVK added to fix output to logUnit
  ...
  INTEGER :: infoUnit=6
!VVK: 09 JULY 2015: infoUnit should be changed to scratchUnit (instead of 6)
!for PR@QE to avoid lengthy output. It is done below at the beginning of each
!subroutine (perhaps not the best solution). This is not working for numSpin==2,
!for that infoUnit==outputUnit (=8)
...
!  REAL(KIND=DP), DIMENSION(SIZE(rhoR,1), SIZE(rhoR,2), &
!                           SIZE(rhoR,3), SIZE(rhoR,4)) :: &
  ...
  INTEGER :: &
    doingSD, &    
    newtonIter=0, & ! Number of CGMIN loops in the newton step !VVK "newtonIter," --> "newtonIter=0,"
  ...
  LOGICAL, SAVE :: printolog = .TRUE. !VVKL to avoid every MD step printing to
                                      !VVK: logUnit, will be changed to .FALSE.
                                      !VVK: after first print


                      !>> INITIALIZATION <<!

  CALL Title("RhoOptimizers::ProjNormRhoMinimization")
  IF(PRatQE) infoUnit = scratchUnit !VVK: redirect infoUnit output for PRatQE  to scratch
  ...
  IF(PRINTOLOG.and.rankGlobal==0) THEN !VVK ADDED, 06 SEP 2015
  WRITE(logUnit, '(/A)') " ------------------------------------------------------------------------------"
  WRITE(logUnit,'(A)')   "                           DENSITY OPTIMIZATION"
  ...
  SELECT CASE(dir_type)
    CASE(1) !NTN
      ftol = 1e-4
      gtol = 2e-1
      !gtol = 1e-2 !VVK use the same as for NCG below
    CASE(2) !NCG
      ftol = 1e-4
      gtol = 1e-2
    CASE(3) !NBF
  ...
  !WRITE(logUnit,*) "numPoints after reduce = ",numPoints
  ...
  DO isp = 1, numSpin
    ...
    IF(PRINTOLOG.and.rankGlobal==0) & !VVK ADDED, 06 SEP 2015
    WRITE(logUnit,*) "Electron number for spin ", isp, " is ",totEleNum(isp)
  ENDDO
  ...
  IF(PRINTOLOG.and.rankGlobal==0) THEN !VVK ADDED, 06 SEP 2015
  WRITE(logUnit,'(A,F20.6,A)') " Cell Volume      : " , cell%vol*bohr**3 , " Angstrom^3"
  WRITE(logUnit,'(A,I20)')     " Ion number       : " , cell%numIon
  WRITE(logUnit,'(A,F20.6)')   " Electron number  : " , Ntot 
  WRITE(logUnit,'(A,F20.6,A)') " Ion density      : " , cell%numIon/cell%vol/bohr**3 , " Ion/Angstrom^3"
  WRITE(logUnit,'(A,F20.6,A)') " Electron density : " , Ntot/cell%vol/bohr**3 , " Electron/Angstrom^3"
  PRINTOLOG = .FALSE. !VVK ADDED, 06 SEP 2015
  ENDIF !VVK ADDED, 06 SEP 2015
...
!  WRITE(logUnit,'(A,I5,A,I5)') " Extra Eleconic Iteration Index", iter_extra, "/", niter_extra
  ...
  DO imag=1,nmag ! Loop for magnetic moment
    ...
    IF(imag>1) THEN
      WRITE(logUnit,*) "New loop for magnetic moment, fixedN=",fixedN
    ENDIF
    ...
    IF (exit_flag .AND. kloc>0._DP) THEN ! (Shin)
      ...
      IF (ABS(olda-aloc)>tolk) THEN 
        WRITE(logUnit,*) " A Parameter in EvW KEDF is ", aloc
        WRITE(logUnit,*) " K Parameter in EvW KEDF is ", kloc
...
! VVK: Above output to logUnit will be repeated every MD step (for PR@QE), hence
! after the first time we replace logUnit by scratchUnit
!  if(PRatQE) logUnit=scratchUnit !VVK
! That line is moved to profess_sub.f90
...
!------------------------------------------------------------------------------
  ...
  DO ! Level 2: electronic iteration
    ...
    !VVK: 14 DEC 2015: theta is added to the list of arguments to stop is theta is too small
    !CALL CheckExit(maxIter,iter,conv_check,e,energy(1),potNorm,rhoOutcome,exit_flag)    
    CALL CheckExit(maxIter,iter,conv_check,e,energy(1),potNorm,rhoOutcome,exit_flag,theta)
    ...
    iter = iter + 1
    ! WRITE(logUnit,*) " Electronic Iteration Number ", iter
...
!------------------------------------------------------------------------------
  ...
  REAL(KIND=DP), INTENT(IN) :: energy(21) ! VVK: changed: 9 --> 21, has to be consistent with dimension in System.f90
...
SUBROUTINE CheckExit(maxIter,i,conv_check,e,e_now,potNorm,rhoOutcome,exit_flag,theta)
!------------------------------------------------------------------------------
! DESCRIPTION:
...
!-------------------------------------------------------------------
  ...
  USE CellInfo, ONLY: cell
  USE CellInfo, ONLY: numSpin !VVK: 14 DEC 2015
  ...
  ! true if exit
  !
  REAL(KIND=DP), DIMENSION(:), INTENT(IN) :: theta !VVK: 14 DEC 2015
  ! value of line step                             !VVK: 14 DEC 2015
  REAL(KIND=DP) :: eps_theta = 1.d-12              !VVK: 14 DEC 2015
  INTEGER, SAVE :: i_theta_small = 0               !VVK: 14 DEC 2015
  INTEGER :: isp                                   !VVK: 14 DEC 2015
  ...
  IF (conv_check(1:4) == "BOTH") THEN
    IF (pConv .and. eConv) THEN !VVK: '.or.' --> '.and.'
      rhoOutcome = 0
      WRITE(message,'(A,Es12.5,A,E12.5,A)')  ' Electronic iteration converged. potTol=', pot_tol, ' and eTol=', tole, ' done!'
      CALL WrtOut(infoUnit,message)
      exit_flag = .TRUE.
    ENDIF
  ENDIF

!--> !VVK ADDED Either as it was in profess2.0
  IF (conv_check(1:4) == "EITH") THEN
    IF (pConv .or. eConv) THEN 
      rhoOutcome = 0
      WRITE(message,'(A,Es12.5,A,E12.5,A)')  ' Electronic iteration converged. potTol=', pot_tol, ' or eTol=', tole, ' done!'
      CALL WrtOut(infoUnit,message)
      exit_flag = .TRUE.
    ENDIF
  ENDIF
  !VVK: 14 DEC 2015
  ! Check for too small line-step two times in a row
  !do isp = 1, numSpin
  do isp = 1,SIZE(theta)
    if(theta(isp).lt.eps_theta) i_theta_small=i_theta_small+1
  enddo
    if(i_theta_small.ge.2*isp) then 
    do isp = 1, numSpin
      WRITE(message,'(A,Es12.5,A,E12.5,A)') ' Line-step is too small: theta=',theta(isp),'<',eps_theta,' two times in a row'
      CALL WrtOut(infoUnit,message)
    enddo
    rhoOutcome = 3 ! 10 JAN 2016
    i_theta_small = 0 !reset counter ! 27 MAR 2016
    exit_flag = .TRUE.
  endif
!<-- VVK
