...
!!!!! BELOW THIS LINE ARE PARAMETERS DIRECTLY MODIFIED BY THIS MODULE !!!!!
  ...
  USE KEDF_VW, ONLY: mu
  USE FS_TF, ONLY: temper !VVK ADDED, MAR 2015
...
! in printing out the parameters.
  ...
  CHARACTER(LEN=4), PARAMETER :: defaultLog = ".log"        ! " " for the log file !VVK
      ...
      CASE("METH")
        ...
        SELECT CASE (TRIM(option(1:3)))
          ...
          CASE("NTN") ! This is sqrt TN method that conserves electron number
            ...
            rhoMethod = 1
            WRITE(message,*) "(input) Energy Minimization Method      : NTN" !VVK
          ...
          CASE("NBF")   ! BFGS density optimizer   (
            WRITE(message,*) "(input) Energy Minimization Method      : NBF=BFGS" !VVK
            CALL WrtOut(6,message)
            rhoMethod = 3
          !---------------------------------------------------
          CASE("STN") ! Select the sqrt truncated newton minimization
            rhoMethod = 4
            WRITE(message,*) "(input) Energy Minimization Method      : STN" !VVK
          CASE("SCG") ! Select the SQRT Conjugate Gradient
            rhoMethod = 5
            WRITE(message,*) "(input) Energy Minimization Method      : SCG" !VVK
          CASE("HYB") ! Select conjugate gradient/newton hybrid
            rhoMethod = 6
            WRITE(message,*) "(input) Energy Minimization Method      : HYB" !VVK
          !---------------------------------------------------
...
! e.g. TTF kinetic=10+1000=1010
          CASE("TTF") !finite-T TF
            kinetic = 1010
            WRITE(message,*) '(input) Non-interacting Free-Energy     : TTF'
      ...
      CASE("PARA")
        ...
        SELECT CASE (TRIM(option(1:4)))
          ...
          CASE("LAMB", "LAM") ! parameter for TF
            lambda = tempReal
            WRITE(message,'('' (input) parameter lambda                : '',E14.6)') lambda !VVK
            CALL WrtOut(6,message) !VVK
          CASE("MU") ! parameter for VW
            mu = tempReal
            WRITE(message,'('' (input) parameter mu                    : '',E14.6)') mu !VVK
            CALL WrtOut(6,message) !VVK
          ! VVK ADDED, MAR 2015
          CASE("NU") ! Setting the parameter nu to its value
              nu = tempReal
            WRITE(message,'('' (input) parameter nu                    : '',E14.6)') nu !VVK
            CALL WrtOut(6,message) !VVK
          CASE("NU2") ! Setting the parameter nu to its value
              nu2 = tempReal
            WRITE(message,'('' (input) parameter nu2                   : '',E14.6)') nu2 !VVK
            CALL WrtOut(6,message) !VVK
          CASE("TEMP","TELE", "TEL") ! Setting the parameter temper to its value
                      ! in modified PROFESS3.0 we define it as electronic
                      ! temperature to distinguish from MD ionic temperature
                      ! (see below "TEMP", "temperature")
            temper = tempReal
            WRITE(message,'('' (input) Electronic Temperature Tel      : '',E14.6, '' K'')') temper
            CALL WrtOut(6,message)
            !outputTemper = tempReal
          ! VVK END
...
!          CALL Error(6, message)
!        END SELECT
        IF(promatches("LDA",TRIM(option))) THEN ! Local Density Approximation (default.)
          CALL WrtOut(6, ' (input) Exchange-Correlation Functional : LDA')
          exchangeCorrelation = 1
        ELSEIF(promatches("PBE",TRIM(option))) THEN ! Generalized Gradient Approximation, PBE version
          CALL WrtOut(6, ' (input) Exchange-Correlation Functional : PBE')
          exchangeCorrelation = 2
        ELSEIF(promatches("PDW00",TRIM(option))) THEN ! PDW 2000 T-dependent XC
          CALL WrtOut(6, ' (input) XC Free-Energy Functional       : PDW00')
          exchangeCorrelation = 3
        ELSEIF(promatches("KSDT",TRIM(option))) THEN ! PDW 2000 T-dependent XC
          CALL WrtOut(6, ' (input) XC Free-Energy Functional       : KSDT')
          exchangeCorrelation = 4
        ELSE
          WRITE(message,*) 'Warning: Unknown EXCH option ',TRIM(option)
          CALL Error(6, message)
        ENDIF
...
!------------------------------------------------------------------------------
  ...
  IF ( nnDist < 0 ) THEN
    ...
    WRITE(6,'(a)') " nnDist < 0, skipped nearest neighbour checking."
  ENDIF
...
! VVK: added from Profess 2.0
!--------------------------------------------------------------------------
! ADDED JAN 2013 from QE
!--------------------------------------------------------------------------
!
